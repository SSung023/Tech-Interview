...
<details>
<summary>자세히</summary>
final로 선언되어 할당되면 그 변수의 값은 변경하지 못합니다. 이를 통해 데이터가 실수로 수정되거나, 악의적인 수정으로부터 보호할 수 있습니다. <br>
메서드나 클래스의 경우에는 오버라이드와 상속을 막아 코드의 안정성을 높여줍니다.<br>
이 외에도 개발자들이 코드를 더 쉽고 명확하게 이해할 수 있으며, JIT 컴파일러가 final로 선언된 클래스나 메서드를 보고 컴파일하는 과정에서 최적화를 진행할 수 있습니다.
</details>
...
<details>
<summary>자세히</summary>
final로 선언된 변수는 상수로 취급되어, 이 변수를 constant pool에 넣거나, 상수로 치환하는 최적화를 진행할 수 있습니다. <br>
컴파일러가 final 변수가 제대로 초기화 되었는지 여부를 확인하며, 그렇지 않은 경우 컴파일 에러를 발생시킵니다.
</details>
