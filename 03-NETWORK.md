...
<details>
<summary>자세히</summary>
Cookie와 session 모두 웹 어플리케이션에서 사용자 정보를 관리하는데에 사용되지만, 몇가지 차이가 있습니다. <br>
먼저 Cookie의 경우 "사용자 브라우저"에 저장되는 작은 데이터 파일로, 서버에 요청할 때마다 자동으로 함께 전송됩니다. <br>
Session의 경우 데이터가 SessionDB에 저장되며, 서버는 사용자에게 고유한 sessionID를 발급하며 이를 cookie를 통해 전달할 수 있습니다 <br>
단, 중요한 데이터는 sessionDB에 저장되어 있지 cookie에 저장되어 있지 않습니다. sessionID를 통해 sessionDB에 있는 정보를 조회할 수 있습니다.

또 Cookie는 클라이언트 측에 저장되기 때문에 데이터가 조작되거나 탈취될 가능성이 있습니다. 따라서 cookie에는 중요한 정보를 담지 않는 것이 좋습니다 <br>
Session의 경우 데이터가 서버 측의 sessionDB에 저장되어 있기 때문에, 상대적으로 안전합니다.
</details>
...
<details>
<summary>자세히</summary>
1. 사용자가 로그인을 한 경우 서버는 사용자의 고유한 session을 생성하고, session에 사용자의 정보를 저장합니다. <br>
2. 서버는 session에 해당하는 고유한 sessionID를 생성하고, 이를 Cookie 형태로 클라이언트에게 전달합니다 <br>
3. 서버는 sessionID를 통해 사용자를 식별하고, 서버에 저장된 데이터에 접근하여 사용자의 로그인 상태에 대한 정보를 확인합니다. <br>
...
<details>
<summary>자세히</summary>
Stateless란, 서버가 클라이언의 "이전 요청을 기억하지 않는 것"을 이야기합니다. <br>
각 요청을 독립적인 요청으로 간주하며, 예를 들어 사용자가 로그인을 완료한 후 -> 다른 페이지에 가서 API를 요청했을 때 서버는 해당 사용자가 로그인을 했는지 여부를 알고 있지 않습니다. <br>
따라서 클라이언트는 서버가 요청을 처리하는데에 필요한 정보를 매번 제공해야 합니다. <br>
이러한 특성 덕분에 서버가 이전의 요청을 저장하고 관리할 필요가 없기 때문에 확장성과 성능을 높이는데 중요한 역할을 합니다.
...
<details>
<summary>자세히</summary>
HTTP의 Stateless와 session은 모순처럼 보이지만, 사실은 session은 stateless를 보완하기 위한 방법입니다. <br>
stateless 특성으로 인해 서버는 이전 요청을 기억하지 않습니다. 따라서 로그인 상태 유지, 장바구니 정보와 같이 사용자의 정보가 필요할 때 session을 사용할 수 있습니다.
</details>

<li> 규모가 커져 서버가 여러 개가 된다면, 세션을 어떻게 관리할 수 있을까요?</li>
<details>
<summary>자세히</summary>
실제로 그와 같은 환경을 경험해본 적은 없지만, 중앙 저장소 방식으로 관리할 수 있을 것 같습니다. <br>
세션 정보를 저장하는 데이터베이스를 하나 두고, 여러 개의 서버들이 이 데이터베이스를 통해 세션 데이터를 관리하도록 합니다. <br>
Redis, Memcached와 같은 인메모리 데이터베이스를 중앙 저장소로 사용하면, 서버가 여러 개 일때에도 동일한 세션 정보를 보장할 수 있을 것 같습니다.
</details>
...
<details>
<summary>자세히</summary>
HTTP 상태 코드는 클라이언트가 서버에 요청을 보낸 후, 서버가 해당 요청을 처리한 결과를 나타내는 숫자 코드입니다. <br>
200번대는 요청이 성공적으로 처리되었음을 말하고, 300번대는 리다이렉션이 필요함을, 400번대는 클라이언트 오류, 500번대는 서버 오류를 이야기합니다
</details>
...
<details>
<summary>자세히</summary>
401과 403은 400번대 응답 코드로서 클라이언트의 오류로 인해 요청을 처리할 수 없음을 나타냅니다. <br>
401 Unauthorized는 사용자의 인증이 되지 않았거나 유효하지 않아 처리가 거부되었음을 나타냅니다. 따라서 정상적인 응답을 받기 위해서는 사용자가 재로그인 등을 통해 유효한 인증을 거쳐야 합니다. <br>
403 Forbidden은 사용자가 정상적으로 인증되었으나, 요청한 리소스로의 접근이 거부됨을 나타냅니다. 권한이 부족하기 때문에 사용자가 재인증을 진행하여도 해당 리소스로의 접근이 막힙니다. <br>
IP를 사용한 블랙리스트, 어드민 페이지로의 접근 방지 등에 사용됩니다.
</details>

<li> 200 (ok) 와 201 (created) 의 차이에 대해 설명해 주세요.</li>
<details>
<summary>자세히</summary>
200과 201은 200번대로서, 클라이언트의 요청이 정상적으로 처리되었음을 나타냅니다. <br>
200 ok는 일반적으로 요청이 정상적으로 처리되었을 때 사용되며, 클라이언트가 요청한 데이터를 담아 보내기도 합니다. <br>
201 created는 서버가 요청을 성공적으로 처리했으며, 새로운 리소스가 성공적으로 생성되었을 때 사용합니다.
</details>
