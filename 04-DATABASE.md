...
<details>
<summary>자세히</summary>
RDB는 관계형 데이터베이스로 행과 열이 있는 테이블의 형태로 데이터를 관리하며, 테이블 간의 관계를 통해 데이터를 연결하고 관리합니다. <br>
고정된 스키마를 따르며 SQL을 사용해 데이터를 관리합니다 <br>
NoSQL은 SQL을 사용하지 않는 모든 데이터베이스를 이야기하며, Graph, Key-Value, Document와 같이 다양한 형태가 있습니다. <br>
RDB와 달리 스키마가 유연하기 때문에 데이터를 저장하는 구조가 변경되어도 추가적인 처리를 하지 않아도 됩니다.
</details>
...
<details>
<summary>자세히</summary>
NoSQL은 고정된 스키마가 없기 때문에 데이터를 저장하는 구조에 변경/확장이 일어나도 추가적인 조치없이 데이터를 저장할 수 있습니다. <br>
또한 join을 통해 복잡한 쿼리로 데이터를 찾아야하는 RDB와 달리, NoSQL은 어플리케이션에서 필요한 형태로 데이터를 저장하기 때문에 비교적 읽기 연산이 빠릅니다. <br>

반면, NoSQL의 유연한 스키마라는 특성으로 인해 저장하는 데이터의 중복이 많이 발생하기 때문에 update 문을 실행했을 때, 해당 값을 가지고 있는 모든 데이터의 값을 부분 수정해야 하므로 update의 속도가 느립니다.
</details>

<li> RDB의 어떠한 특징 때문에 NoSQL에 비해 부하가 많이 걸릴 "수" 있을까요? (주의: 무조건 NoSQL이 RDB 보다 빠르다라고 생각하면 큰일 납니다!)</li>
<details>
<summary>자세히</summary>
우선 RDB는 고정된 스키마로 인해 데이터를 저장하는 구조가 바뀌어 스키마가 변경되는 경우, 데이터가 많이 저장되어 있던 테이블의 경우에 모든 데이터의 구조를 변경하는데에 많은 시간이 걸릴 수 있습니다. <br>
두번째로, RDB는 ACID를 통해 트랜잭션을 제공하는데, 이 때 "동시성 제어"와 "일관성 보장"을 하는 과정에서 오버헤드가 발생할 수 있습니다. <br>
세번째로, RDB는 테이블 간의 Join 연산을 통해 원하는 데이터를 조회하는데, 데이터의 양이 많거나 관계가 복잡해지면 join 연산이 성능 저하의 주요 요인이 될 수 있습니다.
</details>

<li> NoSQL을 활용한 경험이 있나요? 있다면, 왜 RDB를 선택하지 않고 해당 DB를 선택했는지 설명해 주세요.</li>
<details>
<summary>자세히</summary>
JWT의 RTR 전략을 사용하면서, 사용자의 정보와 Refresh token을 짝지어 저장할 때 MongoDB를 사용한 경험이 있습니다. <br>
사용자의 정보를 통해 유효한 Refresh token을 찾는 기능이 필요했는데, Key-Value 형태로 저장하면 빠르게 원하는 데이터를 찾을 수 있을 것이라 기대해 MongoDB를 도입했습니다.
</details>
